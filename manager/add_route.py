...
def trolleybuspark_get():
    ...
    time_out_full = datetime.strptime(f"{date} {time_out}", "%d.%m.%Y %H:%M").strftime("%Y-%m-%d %H:%M:%S")
    time_in_full = datetime.strptime(f"{date} {time_in}", "%d.%m.%Y %H:%M").strftime("%Y-%m-%d %H:%M:%S")
    select_from_db(db_config, sql_provider.get('new_route.sql',
